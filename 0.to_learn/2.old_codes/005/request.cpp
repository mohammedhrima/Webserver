...
void parse_request_header(string &line, Connection &con, Method method)
{
    ...
    // PARSE HTTP version
    if (check(LINE, con, !starts_with(line, "HTTP/"), HTTP_BAD_REQUEST, "expected HTTP/version"))
        return;
    size_t dot = 0;
    ...
    if (check(LINE, con, !_isdigits(l) || !_isdigits(r), HTTP_BAD_REQUEST, "expected HTTP/d.d"))
        return;
...
void parse_header(Connection &con)
{
    ssize_t e, s;
    Data &data = con.data;
    string &buff = con.buff;
    ssize_t pos;
    string sep;

    pos = find_sep(con.buff, sep);
    while (pos != string::npos)
    {
        string request_line = substr(LINE, buff, 0, pos);
        cout << RED << "<" << request_line << ">" << END;
        if (!data.method)
        {
            struct
            {
        ...
        else if (request_line.length())
        {
            ...
            if (epos != string::npos)
            {
                ...
                if (to_lower(key) == "host")
                {
                    ...
                    // TODO: check if host contains valid characters
                    ...
                    // data.hosts.push_front(host);
                    data.hosts.insert(data.hosts.begin(), host);
        ...
        if (request_line.empty())
        {
            ...
            // check for host
            check(LINE, con, !data.host_found, HTTP_BAD_REQUEST, "Host is required");
            // check for content-length
            check(LINE, con, !data.clen_found, HTTP_LENGTH_REQUIRED, "content-Length is missing");
            data.header_parsed = true;
            cout << GREEN;
...
{
    Connection con = {};
    con.buff = "GET http://localhost/abc HTTP/1.1\r\n"
                   "Host: name1\n"
