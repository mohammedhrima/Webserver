...
int main()
{
    ...
    if ((status = getaddrinfo(NULL, SPORT, &hints, &servinfo)) != 0)
    {
    ...
    { // loop for accept incoming connection
        ...
        for (n = 0; n < nfds; ++n)
        {
            ...
            else
            {
                ...
                printf("send\n");
                if (
                    send(events[n].data.fd, header, strlen(header), 0) == -1 ||
                    send(events[n].data.fd, body, strlen(body), 0) == -1)
                {
