...
Socket *new_socket(int fd)
{
    Socket *new = calloc(1, sizeof(Socket));
    new->socket_fd = fd;
...
int main()
{
  ...
  */
    ...
    Socket *head = new_socket(server_fd);
    ...
    while (1)
    {
        ...
        debug("server: waiting for event...\n");
        ...
        for (int i = 0; i < num_events; i++)
        {
            ...
            if (events[i].data.fd == server_fd)
            {
                ...
                sockets[client_fd].socket_fd = client_fd;
            }
            else // EPOLLIN or EPOLLOUT
            {
                ...
                if (events[i].events & EPOLLIN)
                {
                    ...
                    if (bytes > 0)
                    {
                        ...
                        {
                            if (
                                sock->current_length > strlen("GET /") &&
                                memcmp(sock->content, "GET /", strlen("GET /")) == 0)
                            {
                                sock->method = GET_;
                                // FILEPATH
                                s = strlen("GET /");
                                ...
                                s = e;
                                check(
                                    memcmp(sock->content + s, " HTTP/1.0\r\n", strlen(" HTTP/1.0\r\n")) &&
                    ...
                    {
                        check(epoll_ctl(epoll_fd, EPOLL_CTL_DEL, sock->socket_fd, NULL) < 0, "epoll_ctl failed");
                        event.events = EPOLLOUT | EPOLLET;
                        event.data.fd = client_fd;
                        check(epoll_ctl(epoll_fd, EPOLL_CTL_ADD, sock->socket_fd, &event) < 0, "epoll_ctl failed");
                        debug("change event\n");
                    }
                    debug("Exit EPOLL IN\n");
                }
                else if (events[i].events & EPOLLOUT)
                {
                    ...
                    if (sock->method == GET_)
                    {
                        ...
                        {
                            currDir = "./";
                            tmp = strjoin(currDir, sock->filename);
                            debug("open '%s'\n", sock->filename);
                            sock->fileptr = fopen(tmp, "r");
                        ...
                        {
#if 1
                            check((bytes = fread(content, sock->Content_length, 1, sock->fileptr)) < 0, "reading from '%s'", sock->filename);
                            debug("did read '%s'\n", content);
                            // if (bytes)
                            {
                                check((bytes = send(sock->socket_fd, content, sock->Content_length, 0)) < 0, "sending");
                                debug("=============== send %d bytes from body ================\n", bytes);
                                event.events = EPOLLOUT | EPOLLET;
                                event.data.fd = client_fd;
                                check(epoll_ctl(epoll_fd, EPOLL_CTL_MOD, events[i].data.fd, &event) < 0, "epoll_ctl failed");
                                debug("change event\n");
                            }
                            ...
                            // else // bytes == 0
                            // {
                            //     debug("server: connection closed (fd:%d)\n", sock->socket_fd);
                            //     check(epoll_ctl(epoll_fd, EPOLL_CTL_DEL, sock->socket_fd, NULL) < 0, "epoll_ctl failed");
                            //     close(sock->socket_fd);
                            //     // leaks: filename etc ...
                            //     memset(&sockets[events[i].data.fd], 0, sizeof(Socket));
                            // }
                            // exit(1);
#else
                            char *body = "Hello, World!";
                            check((bytes = send(sock->socket_fd, body, strlen(body), 0)) < 0, "sending");
                            check(epoll_ctl(epoll_fd, EPOLL_CTL_DEL, events[i].data.fd, NULL) < 0, "epoll_ctl");
                            close(events[i].data.fd);
#endif
                        }
